...
exports.register = async (req, res, next) => {
  try {
    ...
    // On renvoie le refreshToken en cookie httpOnly (jamais dans le JSON) :
    // un cookie httpOnly est INVISIBLE et INACCESSIBLE depuis le
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    // Réponse IDENTIQUE que l'email existe ou non (voir l'explication dans
    // le service sur l'énumération d'utilisateurs).
    res.status(200).json({
...
exports.me = async (req, res, next) => {
  try {
    ...
    const user = await User.findByPk(req.user.sub, {
      attributes: { exclude: ["passwordUser"] }, // exclut ce champ de la réponse
    });
    if (!user)
...
exports.refreshToken = async (req, res, next) => {
  try {
    // req.cookies existe grâce à cookie-parser (déjà activé dans app.js)
...
// ── NOUVEAU : déconnexion propre (efface le cookie côté navigateur) ──
